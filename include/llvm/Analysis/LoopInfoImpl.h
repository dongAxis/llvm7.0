...
void LoopInfoBase<BlockT, LoopT>::analyze(const DomTreeBase<BlockT> &DomTree) {
  ...
  const DomTreeNodeBase<BlockT> *DomRoot = DomTree.getRootNode();  //支配树的根节点
  for (auto DomNode : post_order(DomRoot)) {   // 后续节点访问

    BlockT *Header = DomNode->getBlock();      // 当前支配节点对应的block节点
