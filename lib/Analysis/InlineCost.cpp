...
void CallAnalyzer::updateThreshold(CallSite CS, Function &Callee) {
  ...
  int LastCallToStaticBonus = InlineConstants::LastCallToStaticBonus;  // 15000
  ...
  if (Caller->optForMinSize()) {   // Attribute::MinSize  (-Oz)
    ...
    SingleBBBonusPercent = 0;
    VectorBonusPercent = 0;
  } else if (Caller->optForSize())   // optimizeForSize (-Os)
    Threshold = MinIfValid(Threshold, Params.OptSizeThreshold);

  // Adjust the threshold based on inlinehint attribute and profile based
  // hotness information if the caller does not have MinSize attribute.
  if (!Caller->optForMinSize()) {    // Os
...
bool CallAnalyzer::analyzeCall(CallSite CS) {
  ...
  Cost -= getCallsiteCost(CS, DL);   // 计算inline的代价
...
int llvm::getCallsiteCost(CallSite CS, const DataLayout &DL) {
  ...
  for (unsigned I = 0, E = CS.arg_size(); I != E; ++I) {
    if (CS.isByValArgument(I)) {   // 如果是byval的参数， 那么意味着存在一个深拷贝
...
InlineCost llvm::getInlineCost(
  ...
  for (unsigned I = 0, E = CS.arg_size(); I != E; ++I)
    if (CS.isByValArgument(I)) {    // 如果当前的参数中存在byval的属性， 那么意味此处存在着一个隐含的s拷贝。
      PointerType *PTy = cast<PointerType>(CS.getArgument(I)->getType());
      if (PTy->getAddressSpace() != AllocaAS)   // 深拷贝参数的as和当前函数不一致， 那么一定不能inline
        return llvm::InlineCost::getNever();
    }

  // Calls to functions with always-inline attributes should be inlined
  // whenever possible.
  if (CS.hasFnAttr(Attribute::AlwaysInline)) {     // 如果被标记为always inline， 那么一定要inline。
  ...
  Function *Caller = CS.getCaller();
  if (!functionsHaveCompatibleAttributes(Caller, Callee, CalleeTTI))   // caller & callee两个函数的attr必须是一致的
    return llvm::InlineCost::getNever();

  // Don't inline this call if the caller has the optnone attribute.
  if (Caller->hasFnAttribute(Attribute::OptimizeNone))    // 如果当前的函数优化等级为o0, 那么永远不Inline
    return llvm::InlineCost::getNever();

  // Don't inline a function that treats null pointer as valid into a caller
  // that does not have this attribute.
  if (!Caller->nullPointerIsDefined() && Callee->nullPointerIsDefined())   //check null pointer
    return llvm::InlineCost::getNever();
  ...
  if (Callee->isInterposable() || Callee->hasFnAttribute(Attribute::NoInline) ||
      CS.isNoInline())  // interposable---> 如果目标函数是可以在运行时被拦截的， 或者不允许inline，那么一定不能被inline
    return llvm::InlineCost::getNever();
