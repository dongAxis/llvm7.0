...
BranchProbabilityInfo::updatePostDominatedByUnreachable(const BasicBlock *BB) {
  ...
  if (TI->getNumSuccessors() == 0) {   // 如果是最好的一个BB， 并且当前BB的终止符号是一个unresearchinst, 那么这个BBb将保存进PostDominatedByUnreachable
  ...
  if (auto *II = dyn_cast<InvokeInst>(TI)) {
    if (PostDominatedByUnreachable.count(II->getNormalDest()))
      PostDominatedByUnreachable.insert(BB);   // 如果当前BB的normal dst已经被证明无法reach, 那么当前BB同样无法reach
    return;
  }

  for (auto *I : successors(BB))    // 如果当前BB所有的后缀边都被证明无法reach, 那么当前的BB同样无法reach。
...
bool BranchProbabilityInfo::calcUnreachableHeuristics(const BasicBlock *BB) {
  ...
  for (succ_const_iterator I = succ_begin(BB), E = succ_end(BB); I != E; ++I)  // 拿到reach和unrearch两个集合
    if (PostDominatedByUnreachable.count(*I))
  ...
  if (ReachableEdges.empty()) {  // 如果所有的分支都不可达， 那么所有的分支的可达性值为1
...
bool BranchProbabilityInfo::calcMetadataWeights(const BasicBlock *BB) {
  ...
  MDNode *WeightsNode = TI->getMetadata(LLVMContext::MD_prof);  // 读取prof中的meta值
  if (!WeightsNode)
    return false;
  ...
  Weights.reserve(TI->getNumSuccessors());  // 计算权重之和， 以及那些后缀是可以到达的
  ...
  if (ScalingFactor > 1) {    // 如果factor大于1， 那么响应的减小每个分支的weight
    WeightSum = 0;
  ...
  if (WeightSum == 0 || ReachableIdxs.size() == 0) {  // update weight sum and weight info for exception
    for (unsigned i = 0, e = TI->getNumSuccessors(); i != e; ++i)
      Weights[i] = 1;
  ...
  if (UnreachableIdxs.size() > 0 && ReachableIdxs.size() > 0) {   // 如果其后缀有部分是不可达， 有部分是可达， 那么进行如下的处理
    auto ToDistribute = BranchProbability::getZero();
    auto UnreachableProb = UR_TAKEN_PROB;
    for (auto i : UnreachableIdxs)
      if (UnreachableProb < BP[i]) {
        ToDistribute += BP[i] - UnreachableProb;    // 这些不可达的后缀权重设置为1， 并且计算拿到全部的原来的权重
    ...
    if (ToDistribute > BranchProbability::getZero()) {    // 将那些不可达的节点的总权重均匀的分给其他的可达节点
  ...
  for (unsigned i = 0, e = TI->getNumSuccessors(); i != e; ++i)
    setEdgeProbability(BB, i, BP[i]);   // 更新节点的权重
...
//    std::cout << "[+]" << F.getName() << std::endl;
  ...
  for (auto BB : post_order(&F.getEntryBlock())) {   // 逆序访问
    LLVM_DEBUG(dbgs() << "Computing probabilities for " << BB->getName()
                      << "\n");
    updatePostDominatedByUnreachable(BB);   // 有哪些是无法访问的
    updatePostDominatedByColdCall(BB);      // 有哪些函数是明显被标记为cold
    // If there is no at least two successors, no sense to set probability.
    if (BB->getTerminator()->getNumSuccessors() < 2)
      continue;
    if (calcMetadataWeights(BB))   // 根据已经存在的prof节点， 计算权重
      continue;
    if (calcInvokeHeuristics(BB))   // 对于invoke指令来说， 这个函数会给normal和unwind两个分支设置一个分数
      continue;
    if (calcUnreachableHeuristics(BB))    // 对于那些后缀数目>1的BB来说， 可以根据unresarch来计算对应的分数
      continue;
    if (calcColdCallHeuristics(BB))      // 同样的， 使用cold来评估
      continue;
