...
AliasResult BasicAAResult::aliasCheck(const Value *V1, LocationSize V1Size,
  ...
  // pointer values are.
  if (V1Size == 0 || V2Size == 0)  // 指针宽度为0， 那么无法做比较
    return NoAlias;

  // Strip off any casts if they exist.
  V1 = V1->stripPointerCastsAndInvariantGroups();  // 尽可能获取到最接近的Pointer Value值
  V2 = V2->stripPointerCastsAndInvariantGroups();

  // If V1 or V2 is undef, the result is NoAlias because we can always pick a
  // value for undef that aliases nothing in the program.
  if (isa<UndefValue>(V1) || isa<UndefValue>(V2))    // 如果当前的pointer都指向了
    return NoAlias;
  ...
  // reach the value.
  if (isValueEqualInPotentialCycles(V1, V2))   // 是否在一个循环中
    return MustAlias;

  if (!V1->getType()->isPointerTy() || !V2->getType()->isPointerTy())  // 如果V1和V2其中一个不是Pointer， 那么就直接返回
    return NoAlias; // Scalars cannot alias each other

  // Figure out what objects these things are pointing to if we can.
  if (O1 == nullptr)
    O1 = GetUnderlyingObject(V1, DL, MaxLookupSearchDepth);  // 拿到一个简化后的Value
