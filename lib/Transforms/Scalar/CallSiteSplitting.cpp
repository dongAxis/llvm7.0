...
static void recordCondition(CallSite CS, BasicBlock *From, BasicBlock *To,
  ...
  auto *BI = dyn_cast<BranchInst>(From->getTerminator());    // convert to br instruction
...
static bool canSplitCallSite(CallSite CS, TargetTransformInfo &TTI) {
  ...
  SmallVector<BasicBlock *, 2> Preds(predecessors(CallSiteBB));   // 只允许存在两个p前缀， 并且没有一个前缀的终止指令是一个jmp
  if (Preds.size() != 2 || isa<IndirectBrInst>(Preds[0]->getTerminator()) ||
      isa<IndirectBrInst>(Preds[1]->getTerminator()))   // jmp
    return false;
...
static bool tryToSplitOnPredicatedArgument(CallSite CS, DominatorTree *DT) {
  auto Preds = getTwoPredecessors(CS.getInstruction()->getParent());   // 经过处理cs指令应该有两个前缀
  if (Preds[0] == Preds[1])
    return false;
