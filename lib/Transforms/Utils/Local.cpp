...
bool llvm::wouldInstructionBeTriviallyDead(Instruction *I,
                                           const TargetLibraryInfo *TLI) {
  if (isa<TerminatorInst>(I))   // 如果当前指令是一个termiator指令， 那么直接返回
    return false;

  // We don't want the landingpad-like instructions removed by anything this
  // general.
  if (I->isEHPad())   // 如果是ExceptionHandle的指令。
    return false;

  // We don't want debug info removed by anything this general, unless
  // debug info is empty.
  if (DbgDeclareInst *DDI = dyn_cast<DbgDeclareInst>(I)) {   // debug instruction
    if (DDI->getAddress())
      return false;
    return true;
  }
  ...
  if (!I->mayHaveSideEffects())  // mayHaveSideEffect--> write memory or throw a exception
    return true;
  ...
  if (isAllocLikeFn(I, TLI))     // 是否调用了分配内存的函数
    return true;

  if (CallInst *CI = isFreeCall(I, TLI))    // 是否释放了内存？
